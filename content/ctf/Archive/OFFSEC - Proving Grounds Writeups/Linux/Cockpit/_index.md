---
date: 2025-08-19
title: Cockpit
---
Difficulty: Intermediate
# OSCP Note Template and Runbooks
This is my checklist of commands and methodologies to use while taking the OSCP. Feel free to use, adapt for your own use, or open a PR with suggestions!

- Clone this [site's repo](https://github.com/CameronCandau/veilcat-site/tree/v4) or copy [my directory structure + runbooks](https://github.com/CameronCandau/veilcat-site/tree/v4/content/OSCP%20Prep/Methodology%20%26%20References/Template) to create a copy this directory for each machine
- Start with [[OSCP Preparation/Notes Template/Service Discovery]]
- Move between the other pages as applicable.


# Resources
- [Red Team Manual: Services Cheat Sheet](https://docs.google.com/document/d/17W30A0wpB7lVTDb7SCjWs0lb9bMAjVR4B7Dp_c2rU2g/edit?tab=t.0#heading=h.aoia5laf4167)
- [Windows Commands Cheat Sheet](https://docs.google.com/document/d/1CGgADAOZQuMXAyzXVeXRNhQ_PPBYliMXCy-4RNE0UMw/edit?tab=t.0)
- [Linux Commands Cheat Sheet](https://docs.google.com/document/d/1vJxoHrjW607NJDLC1Zln1llrEIqrS6Ea3j9ihJTdblg/edit?tab=t.0#heading=h.daugsj3oqt1)
